    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                ...
                <h2>Welcome <?php 
                    session_start();
                    $username = $_SESSION['profFirst'];
                    $professor_id = $_SESSION['professor_id'];
                    echo $username;
                ?> !</h2>
                <h2>Here are comments associated with your classes</h2>
            </div>
        </div>
    </div>
...
<div class="container">
	...
	<table id="commentsTable" class="table table-bordered table-responsive-md table-striped text-center">
		...
		<tbody>
			...
			$sqlQuery = "SELECT comment_id, comment, user_id, course_name, letterGrade, professor_id FROM comment WHERE professor_id=$professor_id";
			$result = mysqli_query($db, $sqlQuery) or die("database error:". mysqli_error($db));
